...
public class Server {
        ...
        try {
            ...
            while (socket.isConnected() && !socket.isClosed()){
		    	...
		    	ByteArrayOutputStream byteArrayOut = new ByteArrayOutputStream();
                        ImageIO.write(takePicture(), "JPEG", byteArrayOut);
			byte[] byteArray = byteArrayOut.toByteArray();
			toClient.writeInt(byteArray.length);
    ...
    public BufferedImage takePicture() throws IOException{
        ...
        switch (getDirection() % 360){
            case 0:
                image = ImageIO.read(new File("./server/image1.jpg"));
                break;
            case 90:
                image = ImageIO.read(new File("./server/image2.jpg"));
                break;
            case 180:
                image = ImageIO.read(new File("./server/image3.jpg"));
                break;
            case 270:
                image = ImageIO.read(new File("./server/image4.jpg"));
                break;
        }
        return image;
    }
